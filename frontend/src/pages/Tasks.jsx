...
// //         params: { search: search || undefined },
// //       }),
// //       client.get(`/organizations/${orgId}/projects/${projectId}/statuses`).catch(() => ({ data: [] })),
// //     ])
// //       .then(([taskRes, statusRes]) => {
// //         setTasks(taskRes.data.items || taskRes.data || []);
// //         const statusData = statusRes.data || [];
// //         setStatuses(statusData);
// //         if (statusData.length > 0 && !statusId) {
// //           setStatusId(statusData[0].id);
// //         }
// //       })
// //       .catch((err) => setError(err.response?.data?.detail || "Failed to load tasks"))
// //       .finally(() => setLoading(false));
// //   }, [orgId, projectId, search, statusId]);
...
// //   const getStatusName = (stId) => {
// //     const found = statuses.find((s) => s.id === stId);
...
// //           {statuses.length === 0 ? (
// //             <p className="text-sm text-amber-600">
// //               No task statuses exist yet for this project. Create project statuses first before adding tasks.
// //             </p>
// //           ) : (
// //             <>
// //               {formError && (
...
// //                         <span className="text-xs px-2 py-1 rounded-md bg-slate-100 text-slate-600 font-medium border border-slate-200">
// //                           {getStatusName(t.status_id)}
// //                         </span>
// //                       )}
...
//         params: { search: search || undefined },
//       }),
//       // Corrected API Route Here:
//       client.get(`/organizations/${orgId}/task-statuses`).catch(() => ({ data: [] })),
//     ])
//       .then(([taskRes, statusRes]) => {
//         setTasks(taskRes.data.items || taskRes.data || []);
//         const statusData = statusRes.data || [];
...
//           {statuses.length === 0 ? (
//             <p className="text-sm text-amber-600">
//               No task statuses exist yet for this organization. Create organization task statuses first before adding tasks.
//             </p>
//           ) : (
//             <>
//               {formError && (
...
export default function Tasks() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      const detail = err.response?.data?.detail;
      setFormError(typeof detail === "string" ? detail : "Failed to create task");
    }
  };
      ...
      <div className="mt-6">
        {loading ? (
          ...
          <Card>
            <ul className="divide-y divide-slate-100">
              {tasks.map((t) => {
                const currentTaskId = t.id || t.task_id;
                return (
                  <li key={currentTaskId || t.title}>
                    <Link
                      to={`/organizations/${orgId}/projects/${projectId}/tasks/${currentTaskId}`}
                      className="py-3 flex justify-between items-center hover:bg-slate-50 -mx-6 px-6 transition-colors"
                    >
                      <span className="text-sm text-slate-700 font-medium">{t.title}</span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-xs px-2 py-1 rounded-full font-medium ${priorityColors[t.priority] || priorityColors.MEDIUM}`}
                        >
                          {t.priority}
                        </span>
                        ...
                        {t.due_date &&
                          (() => {
                            const formatted = formatDueDate(t.due_date);
                            if (!formatted) return null;
                            const { label, isOverdue } = formatted;
                            return (
                              <span
                                className={`text-xs px-2 py-1 rounded-full font-medium ${
                                  isOverdue ? "bg-red-50 text-red-600" : "bg-slate-100 text-slate-500"
