...
#     async def unassign_user(self, *, task_id: uuid.UUID, user_id: uuid.UUID) -> bool:
...
class TaskRepository:
    ...
    async def unassign_user(self, *, task_id: uuid.UUID, user_id: uuid.UUID) -> bool:
        assignee = await self.get_assignment(task_id=task_id, user_id=user_id)
        if assignee:
    ...
    async def list_assigned_to_user(
        ...
        """All tasks (across projects) assigned to a specific user — used by
        the chatbot to answer 'what is X assigned to' for someone other than
        the requester (dashboard's 'my tasks' only covers the current user)."""
        stmt = (
            select(Task)
